...
这种方式在访问路径中包含 OSS-HDFS 服务的 Endpoint，JindoSDK 会根据路径中的 Endpoint 访问对应的 OSS-HDFS 服务接口。 JindoSDK 还支持更多的 Endpoint 配置方式，详情参考[JindoFSx 存储加速/扩展系统配置 OSS-HDFS 服务 Endpoint](../configuration/dls_endpoint_configuration.md)。
...
Found 1 items
----------   1          0 1970-01-01 08:00 fsx://emr-header-1:8101/jindodls
```
即访问`fsx://emr-header-1:8101/jindodls/`等价于访问`oss://<Bucket>.<Endpoint>/`

## 访问 OSS-HDFS 服务
完成上述步骤后作业通过`fsx://`前缀读取 OSS-HDFS 服务上的数据后，会自动缓存到 JindoFSx 存储加速/扩展系统中，后续通过`fsx://`访问相同的数据就能够命中缓存。
