...
Apache HDFS 统一挂载缓存加速可以利用计算集群的闲置存储资源进行数据缓存来加速计算服务，避免了计算集群/服务占用核心集群过多带宽。计算集群或服务使用统一挂载的方式 (jindo://) 访问原 HDFS 集群，同时通过缓存加速访问速度。
...
## 访问 HDFS
完成上述步骤后作业通过`jindo://`前缀读取 HDFS 上的数据后，会自动缓存到 JindoFSx 存储加速/扩展系统中，后续通过`jindo://`访问相同的数据就能够命中缓存。
