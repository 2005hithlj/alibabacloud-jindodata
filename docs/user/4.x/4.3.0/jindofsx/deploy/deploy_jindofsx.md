...
3. [jindofsx-common] 中需配置日志、metrics、credentials、endpoint 相关信息，加速底层存储所需要的 credentials 和 endpoint 配置方法见相应 credential provider 配置文档。
...
如需部署高可用 JindoFSx 存储加速/扩展系统, 请参考 [部署高可用 JindoFSx 存储加速/扩展系统](../deploy/deploy_raft_ns.md)