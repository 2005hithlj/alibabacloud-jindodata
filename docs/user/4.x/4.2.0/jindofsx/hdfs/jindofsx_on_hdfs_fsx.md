...
Apache HDFS 统一挂载缓存加速可以利用计算集群的闲置存储资源进行数据缓存来加速计算服务，避免了计算集群/服务占用核心集群过多带宽。计算集群或服务使用统一挂载的方式 (fsx://) 访问原 HDFS 集群，同时通过缓存加速访问速度。
...
完成以上配置后，作业读取 Apache HDFS 上的数据后，会自动缓存到 JindoFSx 存储加速/扩展系统中，后续访问相同的数据就能够命中缓存。
...
Found 2 items
----------   1          0 1970-01-01 08:00 fsx://emr-header-1:8101/jindohdfs
----------   1          0 1970-01-01 08:00 fsx://emr-header-1:8101/jindooss
```
即访问`fsx://emr-header-1:8101/jindohdfs/`等价于访问`hdfs://emr-header-1:9000/`

## 访问 HDFS
完成上述步骤后作业通过`fsx://`前缀读取 HDFS 上的数据后，会自动缓存到 JindoFSx 存储加速/扩展系统中，后续通过`fsx://`访问相同的数据就能够命中缓存。
